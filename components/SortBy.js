  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          ref={ref}
          className={`absolute top-12 left-0 right-1 z-50 flex max-h-96 w-full flex-col overflow-y-scroll rounded-2xl bg-slate-200 py-4 px-2 shadow`}
        >
          {items.map((item, index) => (
            <span
              key={index}
              className="mb-1 rounded-md py-1 pl-4 text-xs font-semibold tracking-wider text-black hover:bg-slate-400"
            >
